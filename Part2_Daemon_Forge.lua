...
if not _G.Voidware or not _G.Voidware.Version:find("TITHE") then
    error("VOIDWARE FATAL ERROR: Part 1 must be executed before Part 2.")
end
print("VOIDWARE_TITHE(2A): Primordial Engine verified. Proceeding with daemon forge...")

-- // ========================== [ STAGE 1: GODMODE DAEMON FORGE ] ========================== //
print("VOIDWARE_TITHE(2A): Forging the Godmode Daemon...")
...
print("VOIDWARE_TITHE(2A): Forging the Aura Daemon...")
...
-- // =========================== [ STAGE 3: ESP DAEMON FORGE (UNABRIDGED) ] ============================ //
print("VOIDWARE_TITHE(2A): Forging the ESP Daemon...")
...
function ESPEngine:RenderTarget(target, config, nameOverride)
    ...
    local screenPos, onScreen = g_Utils:ToScreen(root.Position); if not onScreen then return end
